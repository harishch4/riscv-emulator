...
export class SystemInterface implements MMIODevice {
  ...
  read(address: number) {
    ...
    } else if ((address & MemoryMap.RAMStart) == MemoryMap.RAMStart) {
  ...
  write(address: number, value: number) {
    if ((address & 0b11) !== 0) {
      throw new Error(`Unaligned write from address 0x${
          toHexString(address)} (value =0x${toHexString(value)})`);
    }
